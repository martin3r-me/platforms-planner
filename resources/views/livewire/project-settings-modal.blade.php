            ...
            <div x-show="activeTab === 'general'" x-transition class="space-y-5">
                ...
                <section class="space-y-2">
                    <div class="flex items-center justify-between">
                        <h4 class="text-[10px] font-semibold uppercase tracking-wider text-[var(--nx-muted)] m-0">Teilnehmer</h4>
                        <span class="text-[10px] text-[var(--nx-muted)] tabular-nums">{{ $project->projectUsers->where('user', '!=', null)->count() }}</span>
                    </div>
                    ...
                    <ul class="space-y-1.5">
                        @foreach($project->projectUsers as $projectUser)
                            @if($projectUser->user)
                                ...
                                @endphp
                                <li class="flex items-center gap-3 px-3 py-2 rounded-lg border border-[color:var(--nx-line)] bg-[color:var(--nx-surface)]">
                                    @if($u->avatar)
                                        <img src="{{ $u->avatar }}" alt="" class="w-8 h-8 rounded-full object-cover flex-shrink-0">
                                    @else
                                        <span class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-[var(--nx-text)] text-white text-[12px] font-semibold flex-shrink-0">{{ $initial }}</span>
                                    @endif
                                    <div class="flex-1 min-w-0">
                                        <div class="text-[13px] font-medium text-[var(--nx-text)] truncate">{{ $u->name }}</div>
                                        <div class="text-[11px] text-[var(--nx-muted)] truncate">{{ $u->email }}</div>
                                    </div>
                                    <div class="flex items-center gap-2 flex-shrink-0">
                                        @if($isOwner)
                                            <span class="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-bold rounded-full bg-[var(--nx-warning)]/10 text-[color:var(--nx-warning)] uppercase tracking-wider">
                                        ...
                                        @else
                                            @can('changeRole', $project)
                                                ...
                                                />
                                            @else
                                                <span class="text-[11px] font-medium px-2 py-0.5 rounded-full bg-[var(--nx-bg)] text-[var(--nx-text)]">
                                            ...
                                            @can('removeMember', $project)
                                                <button
                                                    ...
                                                    class="inline-flex items-center justify-center w-7 h-7 rounded-md text-[var(--nx-muted)] hover:text-[var(--nx-danger)] hover:bg-[var(--nx-danger)]/5 transition-colors"
                                                    title="Entfernen"
                                                >
                    ...
                    @can('invite', $project)
                        ...
                        @if($availableUsers->count() > 0)
                            <details class="rounded-lg border border-[color:var(--nx-line)] bg-[var(--nx-bg)]/40 group">
                                <summary class="px-3 py-2 text-[11px] font-medium text-[var(--nx-text)] cursor-pointer inline-flex items-center gap-1.5 list-none">
                                    @svg('heroicon-o-plus-circle', 'w-3.5 h-3.5 text-[var(--nx-accent)]')
                                    <span>Teilnehmer hinzufügen</span>
                                    <span class="ml-1 text-[10px] text-[var(--nx-muted)] tabular-nums">{{ $availableUsers->count() }} verfügbar</span>
                                </summary>
                                <ul class="px-2 pb-2 space-y-1">
                                    @foreach($availableUsers as $user)
                                        @php $aInitial = mb_strtoupper(mb_substr($user->name ?? $user->email ?? 'U', 0, 1)); @endphp
                                        <li class="flex items-center gap-2 px-2 py-1.5 rounded bg-[color:var(--nx-surface)]">
                                            @if($user->avatar)
                                                <img src="{{ $user->avatar }}" alt="" class="w-6 h-6 rounded-full object-cover flex-shrink-0">
                                            @else
                                                <span class="inline-flex items-center justify-center w-6 h-6 rounded-full bg-[var(--nx-text)] text-white text-[10px] font-semibold flex-shrink-0">{{ $aInitial }}</span>
                                            @endif
                                            <div class="flex-1 min-w-0">
                                                <div class="text-[12px] font-medium text-[var(--nx-text)] truncate">{{ $user->name }}</div>
                                                <div class="text-[10px] text-[var(--nx-muted)] truncate">{{ $user->email }}</div>
                                            </div>
                                            <x-nx-button variant="secondary" size="xs" wire:click="addProjectUser({{ $user->id }}, 'member')">
                                                Hinzufügen
                                            </x-nx-button>
                                        </li>
                                    @endforeach
                        ...
                        @else
                            <p class="text-[11px] text-[var(--nx-muted)] m-0">Alle Team-Mitglieder sind bereits im Projekt.</p>
                        @endif
                    @endcan

                    {{-- Ownership übertragen --}}
                    @can('transferOwnership', $project)
                        <details class="rounded-lg border border-[var(--nx-warning)]/30 bg-[var(--nx-warning)]/10 group">
                            <summary class="px-3 py-2 text-[11px] font-medium text-[color:var(--nx-warning)] cursor-pointer inline-flex items-center gap-1.5 list-none">
                                @svg('heroicon-o-star', 'w-3.5 h-3.5')
                                <span>Ownership übertragen</span>
                            </summary>
                            <div class="px-3 pb-3 space-y-2">
                                <p class="text-[11px] text-[color:var(--nx-warning)] m-0">
                                ...
                                <select
                                    ...
                                    class="w-full text-[12px] border border-[var(--nx-warning)]/30 rounded-md px-2.5 py-1.5 bg-[color:var(--nx-surface)] text-[var(--nx-text)] focus:outline-none focus:ring-2 focus:ring-[var(--nx-warning)]/30"
                                >
