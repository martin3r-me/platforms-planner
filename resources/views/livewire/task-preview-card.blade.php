...
>
    <!-- Story Points und Frosch (oben in eigene Zeile) -->
    @if($isFrog || $task->story_points)
    ...
    <div class="flex items-start gap-2 mb-3 text-xs text-[var(--ui-muted)] min-w-0">
        @if($task->project)
            <span class="inline-flex items-start gap-1 min-w-0">
                @svg('heroicon-o-folder','w-3 h-3 mt-0.5')
        ...
        @if($userInCharge)
            @if($task->project)
                <span class="text-[var(--ui-muted)] mt-0.5">•</span>
            @endif
            <span class="inline-flex items-start gap-1 min-w-0">
