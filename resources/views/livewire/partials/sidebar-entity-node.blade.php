...
<div wire:key="entity-{{ $node['entity_id'] }}"
    ...
    <button type="button"
            ...
            class="flex items-center py-1 px-2 rounded-md text-[var(--ui-secondary)] hover:bg-[var(--ui-muted-5)] transition w-full text-left">
        <span class="w-3.5 h-3.5 flex-shrink-0 flex items-center justify-center transition-transform"
              :class="open ? 'rotate-90' : ''">
            @svg('heroicon-o-chevron-right', 'w-2.5 h-2.5')
        </span>
        @if($typeIcon && str_starts_with($typeIcon, 'heroicon-'))
            @svg($typeIcon, 'w-3.5 h-3.5 flex-shrink-0 ml-0.5 text-[var(--ui-muted)]')
        @else
            @svg('heroicon-o-rectangle-group', 'w-3.5 h-3.5 flex-shrink-0 ml-0.5 text-[var(--ui-muted)]')
        @endif
        <span class="ml-1 text-xs font-medium truncate">{{ $node['entity_name'] }}</span>
        <span class="ml-auto text-[10px] text-[var(--ui-muted)]">{{ $node['total_projects'] }}</span>
    </button>
    <div x-show="open" x-collapse class="flex flex-col pl-3.5">
        ...
        @foreach($node['projects'] as $project)
            <a wire:key="entity-{{ $node['entity_id'] }}-project-{{ $project->id }}"
               ...
               wire:navigate
               title="{{ $project->name }}"
               class="flex items-center gap-1.5 py-0.5 px-2 rounded-md text-[var(--ui-secondary)] hover:bg-[var(--ui-muted-5)] transition">
                @svg('heroicon-o-folder', 'w-3 h-3 flex-shrink-0 text-[var(--ui-muted)]')
        ...
        @foreach($node['children_by_type'] as $typeGroup)
            <div wire:key="entity-{{ $node['entity_id'] }}-type-{{ $typeGroup['type_id'] }}"
                ...
                <button type="button"
                        ...
                        class="flex items-center gap-1 pt-1 pb-0.5 px-2 w-full text-left group">
                    <span class="w-2.5 h-2.5 flex-shrink-0 flex items-center justify-center transition-transform text-[var(--ui-muted)]"
                          :class="groupOpen ? 'rotate-90' : ''">
                        @svg('heroicon-o-chevron-right', 'w-2 h-2')
                    </span>
                    <span class="text-[9px] uppercase tracking-wider text-[var(--ui-muted)] font-semibold group-hover:text-[var(--ui-secondary)] transition-colors">
                        {{ $typeGroup['type_name'] }}
                    </span>
                </button>
