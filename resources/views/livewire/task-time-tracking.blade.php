<div class="space-y-4">
    <div class="rounded-lg border border-[var(--ui-border)]/60 bg-white p-5 shadow-sm">
        <div class="flex flex-col gap-2">
            <div class="flex items-center justify-between">
                <h3 class="text-base font-semibold text-[var(--ui-secondary)]">Zeiterfassung</h3>
                @if($task->planned_minutes)
                    <span class="text-xs text-[var(--ui-muted)]">
                        Plan: {{ number_format($task->planned_minutes / 60, 2, ',', '.') }} h
                    </span>
                @endif
            </div>

            <div class="text-sm text-[var(--ui-secondary)]">
                {{ number_format($this->totalMinutes / 60, 2, ',', '.') }} h erfasst
                @if($this->totalAmountCents)
                    · {{ number_format($this->totalAmountCents / 100, 2, ',', '.') }} €
                @endif
    ...
    <div class="rounded-lg border border-[var(--ui-border)]/60 bg-white shadow-sm">
        <div class="border-b border-[var(--ui-border)]/60 px-5 py-3">
        ...
        <div class="divide-y divide-[var(--ui-border)]/40">
            ...
            @empty
                <div class="px-5 py-5 text-sm text-[var(--ui-muted)]">Noch keine Zeiten erfasst.</div>
            @endforelse
        </div>
    </div>
</div>
