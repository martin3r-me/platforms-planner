<x-ui-page>
    ...
    <x-slot name="actionbar">
        <x-ui-page-actionbar :breadcrumbs="[
            ['label' => 'Planner', 'icon' => 'clipboard-document-list'],
        ]" />
    </x-slot>
            ...
            <x-ui-panel title="Meine aktiven Projekte" subtitle="Top 5 Projekte nach offenen Aufgaben">
                <div class="grid grid-cols-1 gap-3">
                    @forelse($activeProjectsList as $project)
                        @php
                            $href = route('planner.projects.show', ['plannerProject' => $project['id'] ?? null]);
                        @endphp
                        <a href="{{ $href }}" class="flex items-center gap-3 p-3 rounded-md border border-[var(--ui-border)] bg-white hover:bg-[var(--ui-muted-5)] transition">
                            ...
                            <div class="text-sm text-[var(--ui-secondary)] flex items-center gap-3 flex-wrap">
                                <span class="text-[var(--ui-primary)] font-semibold">
                                    {{ number_format(($project['total_minutes'] ?? 0) / 60, 1, ',', '.') }}h
                                </span>
                                @if(isset($project['monthly_minutes']))
                                    <span class="text-[var(--ui-muted)]">
                                        (Monat: {{ number_format(($project['monthly_minutes'] ?? 0) / 60, 1, ',', '.') }}h)
                                    </span>
                                @endif
                                <span><strong>{{ $project['tasks'] ?? 0 }}</strong> Aufgaben</span>
